class Solution:
    """
    Class for the solution to LeetCode Problem 4: Median of Two Sorted Arrays
    """
    ...
    def find_median_sorted_arrays(self, nums1: list[int], nums2: list[int]) -> float:
        ...
        if total_length % 2 == 0:
            ...
            return self.find_recur(
                nums1, 0, len(nums1) - 1, nums2, 0, len(nums2) - 1, total_length // 2
            )
    ...
    def find_recur(
        ...
        nums1_idx, nums2_idx = (nums1_start + nums1_end) // 2, (
            nums2_start + nums2_end
        ) // 2
        ...
        if nums1_length + nums2_length >= k:
            if nums1_median <= nums2_median:
                return self.find_recur(
                    nums1, nums1_start, nums1_end, nums2, nums2_start, nums2_idx - 1, k
                )
            else:
                return self.find_recur(
                    nums1, nums1_start, nums1_idx - 1, nums2, nums2_start, nums2_end, k
                )
